...
export default function AssetForm({ asset, currentUser, admins = [] }: { asset?: Asset, currentUser?: { name: string, role: string, id: string }, admins?: { id: string, name: string }[] }) {
    ...
    const [storageLocation, setStorageLocation] = useState(asset?.StorageLocationID || '')
    ...
    // Manufacturer & Model State (connected to IT Inventory)
    const [manufacturers, setManufacturers] = useState<{ ManufacturerID: string, Name: string }[]>([])
    const [allModels, setAllModels] = useState<{ ModelID: string, Name: string, ManufacturerID: string, Category: string | null, Series: string | null }[]>([])
    const [selectedManufacturer, setSelectedManufacturer] = useState<string>(asset?.Brand || '')
    const [selectedModel, setSelectedModel] = useState<string>(asset?.Model || '')
    const [mfrSearch, setMfrSearch] = useState(asset?.Brand || '')
    const [modelSearch, setModelSearch] = useState(asset?.Model || '')
    ...
    useEffect(() => {
        ...
        // Fetch Manufacturers from IT Inventory
        ...
        // Fetch Asset Models from IT Inventory
        const fetchModels = async () => {
            try {
                const res = await fetch('/api/models')
                if (res.ok) setAllModels(await res.json())
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        data.StorageLocationID = storageLocation || null  // Always send storage location
    ...
    return (
        <form onSubmit={handleSubmit} className="card max-w-4xl mx-auto relative">
            ...
            <div className="mb-8 pt-2">
                ...
                <div className="form-grid">
                    ...
                    <div className="form-group" style={{ position: 'relative' }}>
                        <label className="form-label" htmlFor="Brand">Manufacturer</label>
                        <input type="hidden" name="Brand" value={selectedManufacturer} />
                        <div style={{ position: 'relative' }}>
                            <input
                                ...
                                onBlur={() => setTimeout(() => setMfrOpen(false), 200)}
                                className="input-field"
                                placeholder="Search manufacturer..."
                            />
                        </div>
                        {mfrOpen && manufacturers.length > 0 && (
                            <div style={{
                                position: 'absolute', top: '100%', left: 0, right: 0,
                                zIndex: 999, background: '#fff', border: '1px solid #e5e7eb',
                                borderRadius: '0.5rem', boxShadow: '0 4px 16px rgba(0,0,0,0.1)',
                                maxHeight: '220px', overflowY: 'auto', marginTop: '2px'
                            }}>
                                {manufacturers
                                    .filter(m => m.Name.toLowerCase().includes(mfrSearch.toLowerCase()))
                                    .map(m => (
                                        <div
                                            key={m.ManufacturerID}
                                            onMouseDown={() => {
                                                setSelectedManufacturer(m.Name)
                                                setMfrSearch(m.Name)
                                                setMfrOpen(false)
                                ...
                                {manufacturers.filter(m => m.Name.toLowerCase().includes(mfrSearch.toLowerCase())).length === 0 && (
                                    <div style={{ padding: '8px 12px', color: '#9ca3af', fontSize: '0.875rem' }}>No results</div>
                                )}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="form-group" style={{ position: 'relative' }}>
                        ...
                        {modelOpen && allModels.length > 0 && (
                            <div style={{
                                ...
                                {allModels
                                    .filter(m => {
                                        const mfrMatch = !selectedManufacturer || !manufacturers.find(mfr => mfr.Name === selectedManufacturer) ||
                                            m.ManufacturerID === manufacturers.find(mfr => mfr.Name === selectedManufacturer)?.ManufacturerID
                                        const nameMatch = m.Name.toLowerCase().includes(modelSearch.toLowerCase())
                                        return mfrMatch && nameMatch
                                    })
                                    .map(m => (
                                        <div
                                            key={m.ModelID}
                                        ...
                                        >
                                            ...
                                            {m.Series && <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>{m.Series}</div>}
                                        </div>
                                    ))
                                }
                                {allModels.filter(m => m.Name.toLowerCase().includes(modelSearch.toLowerCase())).length === 0 && (
                                    <div style={{ padding: '8px 12px', color: '#9ca3af', fontSize: '0.875rem' }}>No results</div>
                                )}
                            </div>
                        )}
                    ...
                    {isAssignmentEnabled && (
                        <div className="form-grid animate-fade-in">
                                    ...
                                    >
                                        {admins.map(admin => (
                                            <option key={admin.id} value={admin.id}>
                                                {admin.name} {currentUser && admin.id === currentUser.id ? '(You)' : ''}
                                            </option>
                                        ))}
                                    </select>
                                ) : (
                            ...
                            <div className="form-group">
                                ...
                                <ModernDatePicker
                                    value={assignedDate}
                                    onChange={(e: any) => setAssignedDate(e.target.value)}
                                    className="w-full"
                                />
                            </div>
                            <div className="form-group">
                                ...
                                <ModernDatePicker
                                    ...
                                    onChange={(e: any) => setExpectedReturnDate(e.target.value)}
                                    className="w-full"
                                />
                            </div>
                        </div>
                ...
                <div className="form-grid">
                    ...
                    <div className="form-group">
                        ...
                        <div className="relative">
                            <div className={`${isAssignmentEnabled || !!assignedEmployee ? 'opacity-50 pointer-events-none' : ''}`}>
                                <StatusSelect
                                    value={currentStatusDisplay}
                                    onChange={(val) => !isAssignmentEnabled && setStatus(val)}
                                    statuses={statuses}
                                    allowEmpty={false}
                                />
                            </div>
                            <input type="hidden" name="Status" value={currentStatusDisplay} />
                        </div>
                    ...
                    <div className="form-group mb-4" style={{ gridColumn: '1 / -1' }}>
                        <label className="form-label" htmlFor="StorageLocation">Storage Location</label>
            ...
            {/* Section 4: Warranty & Financial */}
            <div className="mb-8">
                <h3 className="text-lg font-semibold mb-4 border-b pb-2 flex items-center gap-2">
                    🛡️ Warranty &amp; Financial
                </h3>
                <div className="form-grid">
                    <div className="form-group">
                        <label className="form-label" htmlFor="VendorName">Vendor / Supplier</label>
                        <input
                            id="VendorName"
                            name="VendorName"
                            defaultValue={(asset as any)?.VendorName || ''}
                            className="input-field"
                            placeholder="e.g. Lenovo Direct, Dell EMEA"
                        />
                    </div>
                    <div className="form-group">
                        <label className="form-label" htmlFor="VendorContact">Vendor Support Contact</label>
                        <input
                            id="VendorContact"
                            name="VendorContact"
                            defaultValue={(asset as any)?.VendorContact || ''}
                            className="input-field"
                            placeholder="e.g. support@vendor.com or 0800-XXX"
                        />
                    </div>
                    <div className="form-group">
                        <label className="form-label" htmlFor="PurchasePrice">Purchase Price (CHF)</label>
                        <input
                            id="PurchasePrice"
                            name="PurchasePrice"
                            type="number"
                            step="0.01"
                            min="0"
                            defaultValue={(asset as any)?.PurchasePrice ?? ''}
                            className="input-field"
                            placeholder="0.00"
                        />
                    </div>
                    <div className="form-group">
                        ...
                        <ModernDatePicker
                            ...
                            defaultValue={(asset as any)?.WarrantyExpiryDate ? new Date((asset as any).WarrantyExpiryDate).toISOString().split('T')[0] : ''}
                            className="w-full"
                        />
                    </div>
                    <div className="form-group">
                        <label className="form-label" htmlFor="SupportContractEnd">Support Contract Expiry</label>
                        <ModernDatePicker
                            name="SupportContractEnd"
                            defaultValue={(asset as any)?.SupportContractEnd ? new Date((asset as any).SupportContractEnd).toISOString().split('T')[0] : ''}
                            className="w-full"
                        />
                    </div>
                </div>
