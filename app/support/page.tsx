...
export default async function SupportPage() {
    ...
    // Fetch dynamic support contacts from DB
    const { data: supportContacts } = await supabase
        .from("User")
        .select("Username, Email, AvatarUrl, SupportRole")
        .eq("IsSupportContact", true)
    ...
    return (
        <div className="container mx-auto max-w-4xl py-12 px-6">
            ...
            {/* Basel Site Selection */}
            <div className="mt-16 pt-12 border-t border-gray-100">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">Basel IT Services</h2>
                        <div className="flex items-center gap-2 text-gray-500 mt-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-blue-500"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                            <address className="not-italic text-sm">Kirschgartenstrasse 14, 4051 Basel</address>
                        </div>
                    </div>
                    <div className="bg-blue-50 text-blue-700 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest border border-blue-100">
                        Basel Site Hub
                    </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {(supportContacts || []).map((member, i) => (
                        <div key={i} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-all duration-200 group relative">
                            <div className="flex flex-col items-center text-center">
                                <div className="relative mb-4">
                                    <div className="w-24 h-24 rounded-full bg-gray-50 flex items-center justify-center border-4 border-white shadow-sm overflow-hidden">
                                        {member.AvatarUrl ? (
                                            <Image src={member.AvatarUrl} alt={member.Username} fill unoptimized sizes="96px" className="h-full w-full object-cover" />
                                        ) : (
                                ...
                                <h3 className="font-bold text-gray-900 text-lg">{member.Username}</h3>
                                <p className="text-sm font-medium text-blue-600 mb-4">{member.SupportRole || 'IT Support'}</p>
                    ...
                    {(supportContacts || []).length === 0 && (
                        <div className="col-span-full py-12 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                            <p className="text-gray-500">No support contacts have been promoted yet.</p>
                            {user?.role === 'ADMIN' && (
                                <p className="text-sm text-blue-600 mt-2">
                                    <Link href="/admin/users" className="hover:underline">Manage users to show them here</Link>
                                </p>
                            )}
                        </div>
                    )}
