...
export async function GET(req: Request) {
    try {
        ...
        // Build Filters
        let queryBuilder = supabase.from("Asset").select("*, assignments:Assignment(*, Employee(*))");

        if (status) {
            queryBuilder = queryBuilder.eq("Status", status);
        }
        if (type) {
            queryBuilder = queryBuilder.eq("AssetType", type);
        }
        if (startDate && endDate) {
            queryBuilder = queryBuilder.gte("createdAt", new Date(startDate).toISOString())
                .lte("createdAt", new Date(endDate).toISOString());
        }

        // --- FETCH DATA ---

        // 1. All Assets (base for metrics)
        const { data: allAssetsRaw, error } = await queryBuilder;
        ...
        const allAssets = (allAssetsRaw || []).map(a => ({
            ...a,
            assignments: (a.assignments || []).filter((asgn: any) => asgn.Status === 'Active')
        }));
