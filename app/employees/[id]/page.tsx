...
export default async function EmployeeDetailPage(props: { params: Promise<{ id: string }> }) {
    ...
    const { data: empById } = await supabase
        .from("Employee")
        .select("*, assignments:Assignment(*, Asset(*))")
    ...
    // Sort assignments by date desc in memory since nested order in JS client can be tricky
    const allAssignments = (employee.assignments || []).sort((a: any, b: any) => new Date(b.AssignedDate).getTime() - new Date(a.AssignedDate).getTime());
