-- Drop and recreate the assign asset function
...
BEGIN
  ...
  IF v_asset."Status" != 'Available' THEN
    RETURN jsonb_build_object('success', false, 'error', 'Asset is not available for assignment');
  END IF;

  -- Check model stock if asset is linked to a model
